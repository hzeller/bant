...
static BazelPatternBundle GetDependencyGraphDefiningPatterns(
  ...
  // --graph-augment requested to add to fully build dependency graph.
...
CliStatus RunCommand(Session &session, Command cmd,
  ...
  // We don't have a 'Command' object yet, so linear with lots of annoying
  // conditions here.
  ...
  case Command::kCanonicalSources:
...
CliStatus RunCliCommand(Session &session, std::span<std::string_view> args) {
  ...
  static const std::map<std::string_view, Command> kCommandNames = {
    ...
    {"srcs-canonical", Command::kCanonicalSources},
