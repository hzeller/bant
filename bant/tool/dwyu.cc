...
std::set<BazelTarget> DependenciesForIncludes(
  ...
  for (std::string_view s : sources) {
    ...
    auto file_hash_includes = ExtractCCIncludes(*src_content);

    // Now for all includes, we need to make sure we can account for it.
    for (const std::string &inc_file : file_hash_includes) {
      if (IsHeaderInList(inc_file, sources, target_self.package.path)) {
        continue;  // Cool, our own list srcs=[...], hdrs=[...]
      }

      const auto found_in_libs = file_index.headers_from_libs.find(inc_file);
      if (found_in_libs != file_index.headers_from_libs.end()) {
        if (found_in_libs->second != target_self) {
          result.insert(found_in_libs->second);
        }
        continue;
      }

      // Not found anywhere, but maybe we can at least report possible source.
      auto found_in_genrule = file_index.files_from_genrules.find(inc_file);
      if (found_in_genrule != file_index.files_from_genrules.end()) {
        info_out << context.filename << ":"
                 << context.line_columns.GetRange(s)
                 << " '" << source_file << "' has unaccounted #include \""
                 << inc_file << "\"; It is generated by genrule "
                 << found_in_genrule->second.ToString() << ", but not "
                 << "in hdrs=[...] of any cc_library() we depend on.\n";
        *all_headers_accounted_for = false;  // Need to be careful.
        continue;
      }
