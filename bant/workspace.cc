...
static std::optional<int> LoadWorkspaceFromFile(Session &session,
  ...
  query::FindTargets(
    ...
    [&](const query::Result &result) {
      ...
      search_dirs.push_back(absl::StrCat(result.name, "~override"));  // ... 7

      // bazel 8 and 7 unpacking from http_archive() in MODULE.bazel
...
static bool PossiblyApplyBazelIgnore(bant::Filesystem &fs,
  ...
  if (!fs.Exists(bazel_ignore.path())) return false;
