...
#### Status
  ...
  * Outputs in the `list-*` type features provide a neat interface for
    scripting.
  * Bant can't see trough stuff generated by custom bazel rules. So the less
    fancy your BUILD files are (e.g. use a simple `genrule()` instead of custom
    rule), the easier it is for `bant` to grok (and, arguably, humans as well).
